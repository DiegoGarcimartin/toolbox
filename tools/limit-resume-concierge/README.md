...
every 5 min ──▶ launchd runs concierge-sweep.sh (plain bash, no LLM, no app)
                      ...
                      │  reset more than a tick away → exit (free)
                      │  reset less than a tick away → hold, probe 5 s after it
                      │  quota probe rejected → retry every 30 s for 4 min, then next tick
                      │                          (a rejected probe costs nothing)
...
- **Quota gating without spending.** The sweep exits free while the manifests' parsed reset time is more than a tick away — but only if that time is within 5h, the length of the limit window. A reset further out is a mis-parsed timezone, and trusting it would hold every pending session for up to a day; the sweep ignores it and probes instead. After the gate, a minimal probe call decides the pass — while the limit is active the probe is rejected at no cost.
- **Wake up seconds after the reset, not minutes (v3.1).** One probe per 5-minute tick meant a limit that lifted at 17:40 was noticed at 17:45 at best, and any rejected probe cost 5 more minutes. Now the tick that sees the reset less than an interval away holds and probes 5 s after it, and a rejected probe is retried every 30 s for 4 minutes inside the same tick (`CONCIERGE_PROBE_INTERVAL`, `CONCIERGE_PROBE_WINDOW`, `CONCIERGE_RESET_MARGIN` override the defaults). Only an auth failure stops the retries — waiting never fixes that one.
- **The probe's answer is parsed, not tail'ed.** The verdict is the last stdout line that parses as a result object, wherever the CLI puts it, and the rejection reason is logged verbatim. On 2026-09-07 the old `tail -1` read a probe the model had answered "ok" as rejected; the quota had been back on time and the three pending sessions were woken by hand eight minutes later. `Not logged in · Please run /login` now counts as an auth failure too.
- **Locks expire.** The single-instance lock is a directory; a crash or reboot mid-sweep used to leave it behind and every later tick exited with "another sweep is still running", forever, silently. A sweep can hold for a near reset and then probe for its window — about 10 minutes at most — so a lock older than 20 minutes is treated as stale and cleared.
...
- **The wake-up lands within about half a minute of the announced reset** (the tick before it holds and probes 5 s after it, then every 30 s). A limit that lifts off-schedule — no reset hour in the message, or a real reset later than the announced one by more than the 4-minute probe window — is noticed on a later tick, up to 5 minutes after.
